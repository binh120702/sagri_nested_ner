...
class NestedNERDataset(Dataset):
    ...
    def _get_file_path(self, version, mode):
        ...
        if version.startswith("ace"):
            ...
            return f"data/{version}/{mode}.json"
    ...
    def _get_type(self, version):
        ...
        if version.find("sagri") >= 0:
            type_list = ["FAM", "DIS", "AMT", "MIO", "CRP", "LOC", "FOD", "PER", "TIME", "ORG", "AGRI", "TEM", "NUT", "CLI", "ANI", "CHE", "FER", "PRD", "DST", "METHOD", "TARGET", "DATASET", "MATERIAL", "TASK", "METRIC"]
...
def get_cls_num_list(version):
    ...
    if version == "sagri":
        file_path = f"data/sagri/{mode}.json"
    if version == "kbp":
